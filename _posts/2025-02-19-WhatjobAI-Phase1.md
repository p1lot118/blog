...
Before the collection of the data, it imports libraries including:

* **requests** - for making web calls
...
Moreover, it makes sures that it bypasses the gates of the website by defining '**User agent header**'.
...
This code uses two phase approach to collect data.

* **Phase 1 - Getting job ID**
...
It targets specific linkedin "guest" API endpoint (for example, seeMoreJobPostings) that returns a list of jobs for "Data Scientist" jobs in "Melbourne". 

It scans this list, finds the **base-card** for each job and slices out the unique job_id from the **data-entity-urn**.



* **Phase 2 - Getting job details**
...
It loops through the list of IDs and visit a second endpoint - jobPosting/{job_id}.

> This brings up the full, dedicated page for that specific job so you can extract the rich data.

### Data Extractions / Error handling
...
Once on the individual job page, you use **beautifulsoup** to hunt for specific HTML tags and classess like **topcard__title** for the job title.

However, since the web data is messy and some job posts might be missing an applicant count or a description, you should include conditional checks. 


### optimising server request
...
This line pauses the script for a random amount of time between 1 and 3 seconds, mimicking human browsing behaviour.

> This is essential as hitting server with hundreds of requests a second will get you banned..
...
```
Because web scraping is unpredictable and script can crash halfway, saving a backup at the end of every batch (25 jobs) can securely save data.
